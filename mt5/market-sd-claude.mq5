...
void CheckLongEntry(int zoneIndex)
{
    ...
    {
        if(trade.Buy(LotSize, _Symbol, entryPrice, stopLoss, takeProfit, "Structure Long"))
        {
            if(ShowDebugInfo)
...
void CheckShortEntry(int zoneIndex)
{
    ...
    if(riskReward >= MinRiskReward && riskPoints > 0)
    {
        if(trade.Sell(LotSize, _Symbol, entryPrice, stopLoss, takeProfit, "Structure Short"))
        {
            if(ShowDebugInfo)
