//+------------------------------------------------------------------+
//| GBPJPYStrategy 4.5.99H4 EA
//|
//|    Generated by StrategyQuant X Build 142 for MetaTrader
//|    Generated at 08/17/2025 19:33
//| 
//|    Backtested on GBPJPY_M1_dukas / H4, 2009.01.01 - 2022.08.31 
//+------------------------------------------------------------------+
...
input string CustomComment = "GBPJPYStrategy 4.5.99H4";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 22354;        //MagicNumber
...
input int VortexChangesTrnPrd1 = 120;        //VortexChangesTrnPrd1
input int KeltnerChannelPrd1 = 5;        //KeltnerChannelPrd1
input double ProfitTarget1 = 485;        //ProfitTarget1
input double StopLoss1 = 1.6;        //StopLoss1
input double TrailingStopCoef1 = 4;        //TrailingStopCoef1
input int Period1 = 45;        //Period1
input double ProfitTarget2 = 9;        //ProfitTarget2
input double StopLoss2 = 170;        //StopLoss2
input double TrailingStopCoef2 = 1.4;        //TrailingStopCoef2
input double TrailingActivation1 = 50;        //TrailingActivation1
...
#define BOLLINGERBANDS_1 0     //iBands(NULL,0, BBerPeriod1, 0, 1.3, PRICE_CLOSE)
#define VORTEX_1 1     //iCustom(NULL,0, "SqVortex", VortexChangesTrnPrd1)
#define KELTNERCHANNEL_1 2     //iCustom(NULL,0, "SqKeltnerChannel", KeltnerChannelPrd1, 0.9)
#define HIGHEST_1 3     //iCustom(NULL,0, "SqHighest", Period1, PRICE_CLOSE)
#define ATR_1 4     //iCustom(NULL, 0, "SqATR", 65)
#define LOWEST_1 5     //iCustom(NULL,0, "SqLowest", Period1, PRICE_CLOSE)
#define ATR_2 6     //iCustom(NULL, 0, "SqATR", 130)
...
string sqStrategyName = "GBPJPYStrategy 4.5.99H4";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = (sqGetIndicatorValue(BOLLINGERBANDS_1, 2, 1+1)>sqGetIndicatorValue(BOLLINGERBANDS_1, 2, 1));

     ShortEntrySignal = ((sqGetIndicatorValue(VORTEX_1, 0, 2) < sqGetIndicatorValue(VORTEX_1, 1, 2))&&(sqGetIndicatorValue(VORTEX_1, 0, 2+1) > sqGetIndicatorValue(VORTEX_1, 1, 2+1))
      &&   indyCrossesBelow("sqLow(NULL,0, 2)", KELTNERCHANNEL_1, 0, 2, 0, 1));
...
//------------------------
...
)
   {
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 183);
...
//------------------------
...
)
   {
      ...
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, StopLoss2), "Current");
      pt = sqFixMarketPrice(sqGetPTPercentLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, ProfitTarget2), "Current");
      size = mmLots * mmMultiplier;
      ...
      if(_ticket > 0) {
         // order was successfuly placed, set or initialize all its exit methods (SL, PT, Trailing Stop, Exit After Bars, etc.)
         sqSetOrderExpiration(_ticket, 197);

         //Check StopLoss & ProfitTarget
         //sqCheckSLPT(_ticket, sl, pt);

         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "TrailingStopCoef2 * sqGetIndicatorValue(ATR_2, 1)", SLPTTYPE_RANGE);
...
bool initIndicators(){
   ...
   indicatorHandles[BOLLINGERBANDS_1] = iBands(NULL,0, BBerPeriod1, 0, 1.3, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[VORTEX_1] = iCustom(NULL,0, "SqVortex", VortexChangesTrnPrd1);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[KELTNERCHANNEL_1] = iCustom(NULL,0, "SqKeltnerChannel", KeltnerChannelPrd1, 0.9);
   ...
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_2] = iCustom(NULL, 0, "SqATR", 130);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("StopLoss2")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), StopLoss2));
   }
   if(idHash == sqStringHash("TrailingStopCoef2 * sqGetIndicatorValue(ATR_2, 1)")) {
      return (TrailingStopCoef2 * sqGetIndicatorValue(ATR_2, 1));
   }
