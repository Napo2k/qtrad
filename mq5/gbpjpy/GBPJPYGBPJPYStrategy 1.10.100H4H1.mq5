...
input string CustomComment = "GBPJPYStrategy 1.10.100H4";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 22350;        //MagicNumber
...
input double ProfitTarget2 = 185;        //ProfitTarget2
input double StopLoss2 = 185;        //StopLoss2
input double TrailingStop2 = 80;        //TrailingStop2
...
#define BOLLINGERBANDS_1 0     //iBands(NULL,0, BBBarOpensPeriod1, 0, 0.5, PRICE_CLOSE)
...
string sqStrategyName = "GBPJPYStrategy 1.10.100H4";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = (sqOpen("NULL",0, 2) > sqGetIndicatorValue(BOLLINGERBANDS_1, 2, 2+1));

     ShortEntrySignal = (NormalizeDouble((double) sqClose(NULL,0, 1), 6) >= NormalizeDouble((double) sqDaily(NULL,0, "Open", 1), 6));
...
//------------------------
...
)
   {
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 121);
...
//------------------------
...
)
   {
      ...
      if(_ticket > 0) {
         // order was successfuly placed, set or initialize all its exit methods (SL, PT, Trailing Stop, Exit After Bars, etc.)
         sqSetOrderExpiration(_ticket, 184);

         //Check StopLoss & ProfitTarget
         //sqCheckSLPT(_ticket, sl, pt);

         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "TrailingStop2", SLPTTYPE_RANGE);
...
bool initIndicators(){
   ...
   indicatorHandles[BOLLINGERBANDS_1] = iBands(NULL,0, BBBarOpensPeriod1, 0, 0.5, PRICE_CLOSE);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("TrailingStop2")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), TrailingStop2));
   }
