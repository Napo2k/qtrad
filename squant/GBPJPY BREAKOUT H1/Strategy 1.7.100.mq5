...
//| Strategy 1.7.100 EA
//|
//|    Generated by StrategyQuant X Build 143 for MetaTrader5 (netted)
//|    Generated at 01/19/2026 20:55
//| 
//|    Backtested on GBPJPY_M1_dukas / H1,D1,Weekly, 2003.08.04 - 2024.07.11 
//+------------------------------------------------------------------+
...
input string CustomComment = "Strategy_1_7_100";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45687;        //MagicNumber
...
//----------------------------------------------------------------
// WARNING! Template for a trading option  /TradingOptions/LimitOver_variables.tpl missing!
//----------------------------------------------------------------
...
input string Subchart1Symbol = "Same as main chart";
input int Subchart1Timeframe = 1440;
input double Subchart1TickSizeSQ = 0.01; //Subchart1PipSizeSQ
input string Subchart2Symbol = "Same as main chart";
input int Subchart2Timeframe = 10080;
...
#define LOWESTINRANGE_1 0     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqLowestInRange", "12:30", "21:00")
#define BOLLINGERBANDS_1 1     //iBands(NULL,0, 10, 0, 2.25, PRICE_CLOSE)
#define LOWESTINRANGE_2 2     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqLowestInRange", "0:00", "13:30")
#define HIGHESTINRANGE_1 3     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqHighestInRange", "12:30", "21:00")
#define HIGHESTINRANGE_2 4     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqHighestInRange", "0:00", "13:30")
#define BOLLINGERBANDS_2 5     //iBands(NULL,0, 50, 0, 1.9, PRICE_CLOSE)
...
string sqStrategyName = "Strategy 1.7.100";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(LOWESTINRANGE_1, 0, 3), 6) <= NormalizeDouble((double) sqGetIndicatorValue(BOLLINGERBANDS_1, 0 + 1, 1), 6))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(LOWESTINRANGE_2, 0, 1), 6) <= NormalizeDouble((double) sqHigh(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 2), 6)))
      &&   (NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 3), 6) >= NormalizeDouble((double) sqDaily(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Low", 3), 6)))
      &&   (NormalizeDouble((double) sqClose(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 1), 6) > NormalizeDouble((double) sqOpen(NULL,0, 2), 6)));

     ShortEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(HIGHESTINRANGE_1, 0, 3), 6) >= NormalizeDouble((double) sqGetIndicatorValue(BOLLINGERBANDS_1, 1 + 1, 1), 6))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(HIGHESTINRANGE_2, 0, 1), 6) >= NormalizeDouble((double) sqLow(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 2), 6)))
      &&   (NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 3), 6) <= NormalizeDouble((double) sqDaily(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "High", 3), 6)))
      &&   (NormalizeDouble((double) sqClose(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 1), 6) < NormalizeDouble((double) sqOpen(NULL,0, 2), 6)));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Low", 1) + (1.30 * (sqGetIndicatorValue(BOLLINGERBANDS_2, 1, 2)-sqGetIndicatorValue(BOLLINGERBANDS_2, 2, 2)))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 120.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 395.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetExitAfterXBars(_ticket, 48);
      }
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "High", 1) - (1.3 * (sqGetIndicatorValue(BOLLINGERBANDS_2, 1, 2)-sqGetIndicatorValue(BOLLINGERBANDS_2, 2, 2)))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 120), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 395), "Current");
      size = mmLots * mmMultiplier;
...
bool sqHandleTradingOptions() {
   ...
   if(!objLimitOver.onBarUpdate()) continueWithBarUpdate = false;
...
bool initIndicators(){
   ...
   indicatorHandles[LOWESTINRANGE_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqLowestInRange", "12:30", "21:00");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[BOLLINGERBANDS_1] = iBands(NULL,0, 10, 0, 2.25, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LOWESTINRANGE_2] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqLowestInRange", "0:00", "13:30");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HIGHESTINRANGE_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqHighestInRange", "12:30", "21:00");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HIGHESTINRANGE_2] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqHighestInRange", "0:00", "13:30");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[BOLLINGERBANDS_2] = iBands(NULL,0, 50, 0, 1.9, PRICE_CLOSE);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("395.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 395.0));
   }
   if(idHash == sqStringHash("120.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 120.0));
   }
   if(idHash == sqStringHash("395")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 395));
   }
   if(idHash == sqStringHash("120")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 120));
   }
...
bool checkSLPTValid(ENUM_ORDER_TYPE orderType, bool isSL, string symbol, double price, double openPrice){
   ...
   if(isSL){
      if (isBuyOrder && price > sqGetBid(symbol) - minDistance){
         VerboseLog("Based on its logic, the strategy tried to place a long SL in the spread at an incorrect price: ", DoubleToString(price), ", is above bid price : ", DoubleToString(sqGetBid(symbol)));
         return false;      
      } 
      if (!isBuyOrder && price < sqGetAsk(symbol) + minDistance){
         VerboseLog("Based on its logic, the strategy tried to place a short SL in the spread at an incorrect price: ", DoubleToString(price), ", is below ask price : ", DoubleToString(sqGetAsk(symbol)));
         return false;
      }
   }
...
// WARNING! Template for a trading option  /TradingOptions/LimitOver_class.tpl missing, adding empty class:
//----------------------------------------------------------------
class CLimitOver : public CTradingOption {
