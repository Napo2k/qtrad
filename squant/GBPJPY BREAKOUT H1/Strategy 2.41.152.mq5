...
input string CustomComment = "Strategy_2_41_152";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45693;        //MagicNumber
...
#define PARABOLICSAR_1 0     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqParabolicSAR", 0.010, 0.3)
#define LINEARREGRESSION_1 1     //iCustom(NULL,0, "SqLinReg", 14, PRICE_CLOSE)
#define PARABOLICSAR_2 2     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqParabolicSAR", 0.01, 0.3)
#define MTATR_1 3     //iATR(NULL,0, 14)
#define HEIKENASHI_1 4     //255
#define HEIKENASHI_2 5     //255
#define HEIKENASHI_3 6     //255
...
string sqStrategyName = "Strategy 2.41.152";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = (((sqGetBid("Same as main chart") > sqGetIndicatorValue(PARABOLICSAR_1, 3))
      &&   (NormalizeDouble((double) sqHigh(NULL,0, 3), 6) < NormalizeDouble((double) sqDaily(NULL,0, "Close", 1), 6)))
      &&   (NormalizeDouble((double) sqDaily(NULL,0, "Close", 2), 6) < NormalizeDouble((double) sqGetIndicatorValue(LINEARREGRESSION_1, 0, 2), 6)));

     ShortEntrySignal = (((sqGetBid("Same as main chart") < sqGetIndicatorValue(PARABOLICSAR_2, 3))
      &&   (NormalizeDouble((double) sqLow(NULL,0, 3), 6) > NormalizeDouble((double) sqDaily(NULL,0, "Close", 1), 6)))
      &&   (NormalizeDouble((double) sqDaily(NULL,0, "Close", 2), 6) > NormalizeDouble((double) sqGetIndicatorValue(LINEARREGRESSION_1, 0, 2), 6)));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Close", 1) + (2.90 * sqGetIndicatorValue(MTATR_1, 0, 3))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 85.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 365.0), "Current");
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Close", 1) - (2.9 * sqGetIndicatorValue(MTATR_1, 0, 3))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 85), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 365), "Current");
      size = mmLots * mmMultiplier;
      ...
      if(_ticket > 0) {
         ...
         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "50", SLPTTYPE_RANGE);
...
bool initIndicators(){
   ...
   indicatorHandles[PARABOLICSAR_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqParabolicSAR", 0.010, 0.3);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LINEARREGRESSION_1] = iCustom(NULL,0, "SqLinReg", 14, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[PARABOLICSAR_2] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqParabolicSAR", 0.01, 0.3);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("365.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 365.0));
   }
   if(idHash == sqStringHash("85.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 85.0));
   }
   if(idHash == sqStringHash("50.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 50.0));
   }
   if(idHash == sqStringHash("365")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 365));
   }
   if(idHash == sqStringHash("85")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 85));
   }
   if(idHash == sqStringHash("50")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 50));
   }
