...
input string CustomComment = "Strategy_3_62_174";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45697;        //MagicNumber
...
#define VORTEX_1 0     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 120)
#define TRSMA_1 1     //iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE)
#define RSI_1 2     //iRSI(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 20, PRICE_TYPICAL)
#define RSI_2 3     //iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 12, PRICE_LOW)
#define RSI_3 4     //iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 12, PRICE_HIGH)
#define LWMA_1 5     //iMA(NULL,0, 20, 0, MODE_LWMA, PRICE_CLOSE)
...
string sqStrategyName = "Strategy 3.62.174";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 0, 1), 6) <= NormalizeDouble((double) 29.0, 6))
      &&   sqIsUptrend(NULL,0, 0, TRSMA_1))
      &&   (sqGetIndicatorValue(RSI_1, 3)>20.0))
      &&   (sqGetIndicatorValue(RSI_2, 2)>25.0));

     ShortEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 0, 1), 6) >= NormalizeDouble((double) 29, 6))
      &&   sqIsDowntrend(NULL,0, 0, TRSMA_1))
      &&   (sqGetIndicatorValue(RSI_1, 3)<80))
      &&   (sqGetIndicatorValue(RSI_3, 2)<75));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(LWMA_1, 0, 3) + (1.60 * sqBiggestRange("NULL",0, 40, 3))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 195.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 400.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 163);

         //Check StopLoss & ProfitTarget
         //sqCheckSLPT(_ticket, sl, pt);

         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "90.0", SLPTTYPE_RANGE);

         // ExitAfterBars initialization
         sqSetExitAfterXBars(_ticket, 26);
      }
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(LWMA_1, 0, 3) - (1.6 * sqBiggestRange("NULL",0, 40, 3))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 195), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 400), "Current");
      size = mmLots * mmMultiplier;
      ...
      if(_ticket > 0) {
         ...
         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "90", SLPTTYPE_RANGE);
...
bool initIndicators(){
   ...
   indicatorHandles[VORTEX_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 120);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[TRSMA_1] = iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[RSI_1] = iRSI(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 20, PRICE_TYPICAL);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[RSI_2] = iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 12, PRICE_LOW);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[RSI_3] = iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 12, PRICE_HIGH);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LWMA_1] = iMA(NULL,0, 20, 0, MODE_LWMA, PRICE_CLOSE);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("400.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 400.0));
   }
   ...
   if(idHash == sqStringHash("400")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 400));
   }
   if(idHash == sqStringHash("195")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 195));
   }
   if(idHash == sqStringHash("90")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 90));
   }
