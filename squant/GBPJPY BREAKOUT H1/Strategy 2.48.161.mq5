...
input string CustomComment = "Strategy_2_48_161";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45684;        //MagicNumber
...
#define LINEARREGRESSION_1 0     //iCustom(NULL,0, "SqLinReg", 30, PRICE_CLOSE)
#define HIGHESTINRANGE_1 1     //iCustom(NULL,0, "SqHighestInRange", "6:00", "21:30")
#define ATR_1 2     //iCustom(NULL,0, "SqATR", 109)
#define LOWESTINRANGE_1 3     //iCustom(NULL,0, "SqLowestInRange", "6:00", "21:30")
...
string sqStrategyName = "Strategy 2.48.161";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = (((NormalizeDouble((double) sqDaily(NULL,0, "High", 2), 6) < NormalizeDouble((double) sqMonthly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 2), 6))
      &&   (NormalizeDouble((double) sqHigh(NULL,0, 3), 6) < NormalizeDouble((double) sqDaily(NULL,0, "Close", 2), 6)))
      &&   (NormalizeDouble((double) sqDaily(NULL,0, "Close", 3), 6) < NormalizeDouble((double) sqGetIndicatorValue(LINEARREGRESSION_1, 0, 3), 6)));

     ShortEntrySignal = (((NormalizeDouble((double) sqDaily(NULL,0, "Low", 2), 6) > NormalizeDouble((double) sqMonthly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 2), 6))
      &&   (NormalizeDouble((double) sqLow(NULL,0, 3), 6) > NormalizeDouble((double) sqDaily(NULL,0, "Close", 2), 6)))
      &&   (NormalizeDouble((double) sqDaily(NULL,0, "Close", 3), 6) > NormalizeDouble((double) sqGetIndicatorValue(LINEARREGRESSION_1, 0, 3), 6)));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(HIGHESTINRANGE_1, 0, 2) + (0.50 * sqGetIndicatorValue(ATR_1, 0, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 165.0), "Current");
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(LOWESTINRANGE_1, 0, 2) - (0.5 * sqGetIndicatorValue(ATR_1, 0, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 165), "Current");
...
bool initIndicators(){
   ...
   indicatorHandles[LINEARREGRESSION_1] = iCustom(NULL,0, "SqLinReg", 30, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HIGHESTINRANGE_1] = iCustom(NULL,0, "SqHighestInRange", "6:00", "21:30");
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_1] = iCustom(NULL,0, "SqATR", 109);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LOWESTINRANGE_1] = iCustom(NULL,0, "SqLowestInRange", "6:00", "21:30");
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("165.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 165.0));
   }
   if(idHash == sqStringHash("365")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 365));
   }
   if(idHash == sqStringHash("165")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 165));
   }
