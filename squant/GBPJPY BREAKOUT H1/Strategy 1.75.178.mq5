...
input string CustomComment = "Strategy_1_75_178";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45698;        //MagicNumber
...
#define VORTEX_1 0     //iCustom(NULL,0, "SqVortex", 20)
#define ATR_1 1     //iCustom(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "SqATR", 14)
#define HEIKENASHI_1 2     //255
#define HEIKENASHI_2 3     //255
#define HEIKENASHI_3 4     //255
...
string sqStrategyName = "Strategy 1.75.178";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((NormalizeDouble((double) sqMonthly(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "Close", 1), 6) >= NormalizeDouble((double) sqClose(NULL,0, 2), 6))
      &&   sqGetIndicatorValue(VORTEX_1, 0, 2) > sqGetIndicatorValue(VORTEX_1, 1, 2))
      &&   (NormalizeDouble((double) sqDaily(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 3), 6) >= NormalizeDouble((double) sqMonthly(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "Close", 1), 6)))
      &&   sqIsFalling(ATR_1, 2, false, 1));

     ShortEntrySignal = ((((NormalizeDouble((double) sqMonthly(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "Close", 1), 6) <= NormalizeDouble((double) sqClose(NULL,0, 2), 6))
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Open", 1) + (1.70 * sqBiggestRange("NULL",0, 50, 2))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 175.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 265.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetExitAfterXBars(_ticket, 20);
      }
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Open", 1) - (1.7 * sqBiggestRange("NULL",0, 50, 2))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 175), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 265), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[VORTEX_1] = iCustom(NULL,0, "SqVortex", 20);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_1] = iCustom(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "SqATR", 14);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("265.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 265.0));
   }
   if(idHash == sqStringHash("175.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 175.0));
   }
   if(idHash == sqStringHash("265")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 265));
   }
   if(idHash == sqStringHash("175")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 175));
   }
