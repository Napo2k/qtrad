...
input string CustomComment = "Strategy_3_89_153";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45688;        //MagicNumber
...
#define MOVINGAVERAGE_1 0     //iMA(NULL,0, 40, 0, MODE_SMMA, PRICE_CLOSE)
#define TRSMA_1 1     //iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE)
#define MOVINGAVERAGE_2 2     //iMA(NULL,0, 30, 0, MODE_LWMA, PRICE_CLOSE)
#define HIGHEST_1 3     //iCustom(NULL,0, "SqHighest", 50, PRICE_HIGH)
#define BOLLINGERBANDS_1 4     //iBands(NULL,0, 10, 0, 1.9, PRICE_CLOSE)
#define ATR_1 5     //iCustom(NULL, 0, "SqATR", 65)
#define LOWEST_1 6     //iCustom(NULL,0, "SqLowest", 50, PRICE_LOW)
...
string sqStrategyName = "Strategy 3.89.153";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((sqClose("NULL",0, 2) > sqGetIndicatorValue(MOVINGAVERAGE_1, 2))
      &&   sqIsUptrend(NULL,0, 0, TRSMA_1))
      &&   sqIsUptrend(NULL,0, 0, TRSMA_1))
      &&   (sqClose("NULL",0, 2) > sqGetIndicatorValue(MOVINGAVERAGE_2, 2)));

     ShortEntrySignal = ((((sqClose("NULL",0, 2) < sqGetIndicatorValue(MOVINGAVERAGE_1, 2))
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(HIGHEST_1, 0, 2) + (2.10 * (sqGetIndicatorValue(BOLLINGERBANDS_1, 1, 2)-sqGetIndicatorValue(BOLLINGERBANDS_1, 2, 2)))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 95.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 300.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 28);

         //Check StopLoss & ProfitTarget
         //sqCheckSLPT(_ticket, sl, pt);

         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "2.6 * sqGetIndicatorValue(ATR_1, 1)", SLPTTYPE_RANGE);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(LOWEST_1, 0, 2) - (2.1 * (sqGetIndicatorValue(BOLLINGERBANDS_1, 1, 2)-sqGetIndicatorValue(BOLLINGERBANDS_1, 2, 2)))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 95), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 300), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[MOVINGAVERAGE_1] = iMA(NULL,0, 40, 0, MODE_SMMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[TRSMA_1] = iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MOVINGAVERAGE_2] = iMA(NULL,0, 30, 0, MODE_LWMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HIGHEST_1] = iCustom(NULL,0, "SqHighest", 50, PRICE_HIGH);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[BOLLINGERBANDS_1] = iBands(NULL,0, 10, 0, 1.9, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_1] = iCustom(NULL, 0, "SqATR", 65);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LOWEST_1] = iCustom(NULL,0, "SqLowest", 50, PRICE_LOW);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("300.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 300.0));
   }
   if(idHash == sqStringHash("95.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 95.0));
   }
   if(idHash == sqStringHash("2.6 * sqGetIndicatorValue(ATR_1, 1)")) {
      return (2.6 * sqGetIndicatorValue(ATR_1, 1));
   }
   if(idHash == sqStringHash("300")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 300));
   }
   if(idHash == sqStringHash("95")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 95));
   }
   if(idHash == sqStringHash("2.6 * sqGetIndicatorValue(ATR_1, 1)")) {
      return (2.6 * sqGetIndicatorValue(ATR_1, 1));
   }
