...
input string CustomComment = "Strategy_2_45_172";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45690;        //MagicNumber
...
#define STOCHASTIC_1 0     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqStochastic", 14, 7, 3, MODE_SMMA, STO_LOWHIGH)
#define KELTNERCHANNEL_1 1     //iCustom(NULL,0, "SqKeltnerChannel", 104, 2.7)
#define PARABOLICSAR_1 2     //iCustom(NULL,0, "SqParabolicSAR", 0.001, 0.20)
#define PARABOLICSAR_2 3     //iCustom(NULL,0, "SqParabolicSAR", 0.001, 0.2)
...
string sqStrategyName = "Strategy 2.45.172";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((sqIsRising(STOCHASTIC_1, 2, false, 2, 1)
      &&   (sqOpen("NULL",0, 1) > sqGetIndicatorValue(KELTNERCHANNEL_1, 1, 1+1)))
      &&   (sqGetBid("NULL") > sqGetIndicatorValue(PARABOLICSAR_1, 2)));

     ShortEntrySignal = ((sqIsFalling(STOCHASTIC_1, 2, false, 2, 1)
      &&   (sqOpen("NULL",0, 1) < sqGetIndicatorValue(KELTNERCHANNEL_1, 0, 1+1)))
      &&   (sqGetBid("NULL") < sqGetIndicatorValue(PARABOLICSAR_2, 2)));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice(sqClose(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 3), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 170.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 375.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 5);
...
//------------------------
...
)
   {
      ...
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 375), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[STOCHASTIC_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqStochastic", 14, 7, 3, MODE_SMMA, STO_LOWHIGH);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[KELTNERCHANNEL_1] = iCustom(NULL,0, "SqKeltnerChannel", 104, 2.7);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[PARABOLICSAR_1] = iCustom(NULL,0, "SqParabolicSAR", 0.001, 0.20);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[PARABOLICSAR_2] = iCustom(NULL,0, "SqParabolicSAR", 0.001, 0.2);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("375.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 375.0));
   }
   if(idHash == sqStringHash("170.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 170.0));
   }
   if(idHash == sqStringHash("375")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 375));
   }
