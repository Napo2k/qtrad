...
input string CustomComment = "Strategy_1_22_192";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45691;        //MagicNumber
...
#define VORTEX_1 0     //iCustom(NULL,0, "SqVortex", 90)
#define ATR_1 1     //iCustom(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "SqATR", 40)
#define ICHIMOKU_1 2     //iCustom(NULL,0, "SqIchimoku", 9, 26, 52)
#define BOLLINGERBANDS_1 3     //iBands(NULL,0, 106, 0, 1.0, PRICE_CLOSE)
#define BOLLINGERBANDS_2 4     //iBands(NULL,0, 106, 0, 1, PRICE_CLOSE)
...
string sqStrategyName = "Strategy 1.22.192";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((NormalizeDouble((double) sqClose(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 2), 6) > NormalizeDouble((double) sqOpen(NULL,0, 2), 6))
      &&   sqGetIndicatorValue(VORTEX_1, 0, 2) > sqGetIndicatorValue(VORTEX_1, 1, 2))
      &&   (NormalizeDouble((double) sqClose(NULL,0, 3), 6) >= NormalizeDouble((double) sqDaily(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 3), 6)))
      &&   sqIsFalling(ATR_1, 2, false, 2));

     ShortEntrySignal = ((((NormalizeDouble((double) sqClose(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 2), 6) < NormalizeDouble((double) sqOpen(NULL,0, 2), 6))
      &&   sqGetIndicatorValue(VORTEX_1, 1, 2) > sqGetIndicatorValue(VORTEX_1, 0, 2))
      &&   (NormalizeDouble((double) sqClose(NULL,0, 3), 6) <= NormalizeDouble((double) sqDaily(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 3), 6)))
      &&   sqIsFalling(ATR_1, 2, false, 2));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(ICHIMOKU_1, 0, 2) + (2.60 * (sqGetIndicatorValue(BOLLINGERBANDS_1, 1, 2)-sqGetIndicatorValue(BOLLINGERBANDS_1, 2, 2)))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 135.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 305.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 67);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(ICHIMOKU_1, 0, 2) - (2.6 * (sqGetIndicatorValue(BOLLINGERBANDS_2, 1, 2)-sqGetIndicatorValue(BOLLINGERBANDS_2, 2, 2)))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 135), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 305), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[VORTEX_1] = iCustom(NULL,0, "SqVortex", 90);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_1] = iCustom(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "SqATR", 40);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ICHIMOKU_1] = iCustom(NULL,0, "SqIchimoku", 9, 26, 52);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[BOLLINGERBANDS_1] = iBands(NULL,0, 106, 0, 1.0, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[BOLLINGERBANDS_2] = iBands(NULL,0, 106, 0, 1, PRICE_CLOSE);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("305.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 305.0));
   }
   if(idHash == sqStringHash("135.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 135.0));
   }
   if(idHash == sqStringHash("305")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 305));
   }
   if(idHash == sqStringHash("135")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 135));
   }
