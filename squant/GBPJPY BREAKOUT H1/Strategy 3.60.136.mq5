...
input string CustomComment = "Strategy_3_60_136";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45695;        //MagicNumber
...
#define VORTEX_1 0     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 191)
#define TRSMA_1 1     //iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE)
#define MOVINGAVERAGE_1 2     //iMA(NULL,0, 20, 0, MODE_LWMA, PRICE_CLOSE)
#define RSI_1 3     //iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 14, PRICE_LOW)
#define RSI_2 4     //iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 14, PRICE_HIGH)
#define EMA_1 5     //iMA(NULL,0, 14, 0, MODE_EMA, PRICE_OPEN)
...
string sqStrategyName = "Strategy 3.60.136";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 0, 3), 6) <= NormalizeDouble((double) 16.0, 6))
      &&   sqIsUptrend(NULL,0, 0, TRSMA_1))
      &&   (sqClose("NULL",0, 2) > sqGetIndicatorValue(MOVINGAVERAGE_1, 2)))
      &&   (sqGetIndicatorValue(RSI_1, 2)>25.0));

     ShortEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 0, 3), 6) >= NormalizeDouble((double) 16, 6))
      &&   sqIsDowntrend(NULL,0, 0, TRSMA_1))
      &&   (sqClose("NULL",0, 2) < sqGetIndicatorValue(MOVINGAVERAGE_1, 2)))
      &&   (sqGetIndicatorValue(RSI_2, 2)<75));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(EMA_1, 0, 2) + (1.50 * sqBiggestRange("NULL",0, 40, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 120.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 215.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 132);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqGetIndicatorValue(EMA_1, 0, 2) - (1.5 * sqBiggestRange("NULL",0, 40, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 120), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 215), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[VORTEX_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 191);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[TRSMA_1] = iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MOVINGAVERAGE_1] = iMA(NULL,0, 20, 0, MODE_LWMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[RSI_1] = iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 14, PRICE_LOW);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[RSI_2] = iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 14, PRICE_HIGH);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[EMA_1] = iMA(NULL,0, 14, 0, MODE_EMA, PRICE_OPEN);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("215.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 215.0));
   }
   if(idHash == sqStringHash("120.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 120.0));
   }
   if(idHash == sqStringHash("215")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 215));
   }
