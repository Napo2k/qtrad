...
input string CustomComment = "Strategy_2_4_174";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45686;        //MagicNumber
...
#define ADX_1 0     //iCustom(NULL,0, "SqADX", 30)
#define TEMA_1 1     //iTEMA(NULL,0, 20, 0, PRICE_WEIGHTED)
#define SUPERTREND_1 2     //iCustom(NULL,0, "SqSuperTrend", 1, 100,9.2)
#define HIGHEST_1 3     //iCustom(NULL,0, "SqHighest", 30, PRICE_HIGH)
#define LOWEST_1 4     //iCustom(NULL,0, "SqLowest", 30, PRICE_LOW)
...
string sqStrategyName = "Strategy 2.4.174";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((changesUp(ADX_1, 2, 0)
      &&   (NormalizeDouble((double) sqGetIndicatorValue(TEMA_1, 0, 1), 6) > NormalizeDouble((double) sqGetIndicatorValue(SUPERTREND_1, 0, 2), 6)))
      &&   (NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Open", 2), 6) > NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 3), 6)));

     ShortEntrySignal = ((changesUp(ADX_1, 2, 0)
      &&   (NormalizeDouble((double) sqGetIndicatorValue(TEMA_1, 0, 1), 6) < NormalizeDouble((double) sqGetIndicatorValue(SUPERTREND_1, 0, 2), 6)))
      &&   (NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Open", 2), 6) < NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Close", 3), 6)));
...
//------------------------
...
)
   {
      ...
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 280.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 126);

         //Check StopLoss & ProfitTarget
         //sqCheckSLPT(_ticket, sl, pt);

         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "100.0", SLPTTYPE_RANGE);
         sqSetTSActivation(_ticket, "70.0");
...
//------------------------
...
)
   {
      ...
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 280), "Current");
      size = mmLots * mmMultiplier;
      ...
      if(_ticket > 0) {
         ...
         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "100", SLPTTYPE_RANGE);
         sqSetTSActivation(_ticket, "70");
...
bool initIndicators(){
   ...
   indicatorHandles[ADX_1] = iCustom(NULL,0, "SqADX", 30);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[TEMA_1] = iTEMA(NULL,0, 20, 0, PRICE_WEIGHTED);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[SUPERTREND_1] = iCustom(NULL,0, "SqSuperTrend", 1, 100,9.2);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[HIGHEST_1] = iCustom(NULL,0, "SqHighest", 30, PRICE_HIGH);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[LOWEST_1] = iCustom(NULL,0, "SqLowest", 30, PRICE_LOW);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("280.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 280.0));
   }
   if(idHash == sqStringHash("195.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 195.0));
   }
   if(idHash == sqStringHash("100.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 100.0));
   }
   if(idHash == sqStringHash("70.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 70.0));
   }
   if(idHash == sqStringHash("280")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 280));
   }
   if(idHash == sqStringHash("195")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 195));
   }
   if(idHash == sqStringHash("100")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 100));
   }
   if(idHash == sqStringHash("70")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 70));
   }
