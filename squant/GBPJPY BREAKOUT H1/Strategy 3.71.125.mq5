...
input string CustomComment = "Strategy_3_71_125";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45678;        //MagicNumber
...
#define VORTEX_1 0     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 160)
#define VORTEX_2 1     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 67)
#define VORTEX_3 2     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 40)
#define RSI_1 3     //iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 20, PRICE_CLOSE)
#define TRSMA_1 4     //iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE)
#define ATR_1 5     //iCustom(NULL, 0, "SqATR", 90)
...
string sqStrategyName = "Strategy 3.71.125";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 1, 3), 6) <= NormalizeDouble((double) sqGetIndicatorValue(VORTEX_2, 0, 3), 6))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(VORTEX_3, 0, 3), 6) <= NormalizeDouble((double) 8.0, 6)))
      &&   (sqGetIndicatorValue(RSI_1, 1)>25.0))
      &&   sqIsUptrend(NULL,0, 0, TRSMA_1));

     ShortEntrySignal = ((((NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 1, 3), 6) >= NormalizeDouble((double) sqGetIndicatorValue(VORTEX_2, 0, 3), 6))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(VORTEX_3, 0, 3), 6) >= NormalizeDouble((double) 8, 6)))
      &&   (sqGetIndicatorValue(RSI_1, 1)<75))
      &&   sqIsDowntrend(NULL,0, 0, TRSMA_1));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqOpen(NULL,0, 1) + (1.20 * sqBiggestRange("NULL",0, 25, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 85.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 340.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 157);

         //Check StopLoss & ProfitTarget
         //sqCheckSLPT(_ticket, sl, pt);

         // TrailingStop initialization
         sqSetTrailingStop(_ticket, "2.8 * sqGetIndicatorValue(ATR_1, 1)", SLPTTYPE_RANGE);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqOpen(NULL,0, 1) - (1.2 * sqBiggestRange("NULL",0, 25, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 85), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 340), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[VORTEX_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 160);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[VORTEX_2] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 67);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[VORTEX_3] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 40);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[RSI_1] = iRSI(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), 20, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[TRSMA_1] = iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_1] = iCustom(NULL, 0, "SqATR", 90);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("340.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 340.0));
   }
   if(idHash == sqStringHash("85.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 85.0));
   }
   if(idHash == sqStringHash("2.8 * sqGetIndicatorValue(ATR_1, 1)")) {
      return (2.8 * sqGetIndicatorValue(ATR_1, 1));
   }
   if(idHash == sqStringHash("340")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 340));
   }
