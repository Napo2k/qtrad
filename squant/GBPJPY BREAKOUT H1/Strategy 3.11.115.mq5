...
input string CustomComment = "Strategy_3_11_115";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45680;        //MagicNumber
...
#define KELTNERCHANNEL_1 0     //iCustom(NULL,0, "SqKeltnerChannel", 20, 1.9)
...
#define MTATR_1 4     //iATR(NULL,0, 30)
...
string sqStrategyName = "Strategy 3.11.115";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = (((sqIsFalling(KELTNERCHANNEL_1, 2, false, 1, 1)
      &&   (NormalizeDouble((double) sqOpen(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 3), 6) > NormalizeDouble((double) sqGetIndicatorValue(LOWESTINRANGE_1, 0, 1), 6)))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(TEMA_1, 0, 2), 6) <= NormalizeDouble((double) sqDaily(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "High", 2), 6)))
      &&   (NormalizeDouble((double) sqDaily(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "Low", 1), 6) >= NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "Low", 1), 6)));

     ShortEntrySignal = (((sqIsRising(KELTNERCHANNEL_1, 2, false, 1, 0)
      &&   (NormalizeDouble((double) sqOpen(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 3), 6) < NormalizeDouble((double) sqGetIndicatorValue(HIGHESTINRANGE_1, 0, 1), 6)))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(TEMA_1, 0, 2), 6) >= NormalizeDouble((double) sqDaily(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "Low", 2), 6)))
      &&   (NormalizeDouble((double) sqDaily(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "High", 1), 6) <= NormalizeDouble((double) sqWeekly(correctSymbol(Subchart2Symbol),TFMigrate(Subchart2Timeframe), "High", 1), 6)));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "High", 3) + (2.10 * sqGetIndicatorValue(MTATR_1, 0, 2))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 170.0), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 290.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 85);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Low", 3) - (2.1 * sqGetIndicatorValue(MTATR_1, 0, 2))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 170), "Current");
      pt = sqFixMarketPrice(sqGetPTLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 290), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[KELTNERCHANNEL_1] = iCustom(NULL,0, "SqKeltnerChannel", 20, 1.9);
   ...
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MTATR_1] = iATR(NULL,0, 30);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("290.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 290.0));
   }
   if(idHash == sqStringHash("170.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 170.0));
   }
   if(idHash == sqStringHash("290")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 290));
   }
   if(idHash == sqStringHash("170")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 170));
   }
