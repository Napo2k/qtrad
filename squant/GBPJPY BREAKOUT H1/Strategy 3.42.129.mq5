...
input string CustomComment = "Strategy_3_42_129";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 45681;        //MagicNumber
...
#define SUPERTREND_1 0     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqSuperTrend", 1, 120,3.2)
#define VORTEX_1 1     //iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 40)
#define MOVINGAVERAGE_1 2     //iMA(NULL,0, 40, 0, MODE_SMMA, PRICE_CLOSE)
#define TRSMA_1 3     //iMA(NULL,0, 200, 0, MODE_SMA, PRICE_CLOSE)
...
string sqStrategyName = "Strategy 3.42.129";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = ((((sqClose(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 0) < sqGetIndicatorValue(SUPERTREND_1, 0))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 0, 2), 6) <= NormalizeDouble((double) 38.0, 6)))
      &&   (sqClose("NULL",0, 1) > sqGetIndicatorValue(MOVINGAVERAGE_1, 1)))
      &&   sqIsUptrend(NULL,0, 0, TRSMA_1));

     ShortEntrySignal = ((((sqClose(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), 0) > sqGetIndicatorValue(SUPERTREND_1, 0))
      &&   (NormalizeDouble((double) sqGetIndicatorValue(VORTEX_1, 0, 2), 6) >= NormalizeDouble((double) 38, 6)))
      &&   (sqClose("NULL",0, 1) < sqGetIndicatorValue(MOVINGAVERAGE_1, 1)))
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "High", 3) + (0.70 * sqBarRange("NULL",0, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 125.0), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 26);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Low", 3) - (0.7 * sqBarRange("NULL",0, 1))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 125), "Current");
...
bool initIndicators(){
   ...
   indicatorHandles[SUPERTREND_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqSuperTrend", 1, 120,3.2);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[VORTEX_1] = iCustom(correctSymbol(Subchart1Symbol),TFMigrate(Subchart1Timeframe), "SqVortex", 40);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("125.0")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 125.0));
   }
   if(idHash == sqStringHash("395")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 395));
   }
   if(idHash == sqStringHash("125")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 125));
   }
