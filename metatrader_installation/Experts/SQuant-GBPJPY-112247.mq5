...
input string CustomComment = "Strategy_1_82_129";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 112247;        //MagicNumber
...
#define ATR_1 0     //iCustom(NULL,0, "SqATR", 14)
...
string sqStrategyName = "Strategy 1.82.129";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = (((sqTimeHour(sqTime(NULL,0, 3)) < 6)
      &&   (sqTimeHour(sqTime(NULL,0, 1)) < 3))
      &&   (sqTimeHour(sqTime(NULL,0, 3)) < 9));

     ShortEntrySignal = (((sqTimeHour(sqTime(NULL,0, 3)) < 6)
      &&   (sqTimeHour(sqTime(NULL,0, 1)) < 3))
      &&   (sqTimeHour(sqTime(NULL,0, 3)) < 9));
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Close", 2) + (2.60 * sqGetIndicatorValue(ATR_1, 0, 3))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 75.0), "Current");
      pt = sqFixMarketPrice(sqGetPTPercentLevel("Current", ORDER_TYPE_BUY_STOP, openPrice, 1, 6.7), "Current");
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqDaily(NULL,0, "Close", 2) - (2.6 * sqGetIndicatorValue(ATR_1, 0, 3))), "Current");
      sl = sqFixMarketPrice(sqGetSLLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 75), "Current");
      pt = sqFixMarketPrice(sqGetPTPercentLevel("Current", ORDER_TYPE_SELL_STOP, openPrice, 1, 6.7), "Current");
      size = mmLots * mmMultiplier;
...
bool initIndicators(){
   ...
   indicatorHandles[ATR_1] = iCustom(NULL,0, "SqATR", 14);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("75")) {
      return (sqConvertToRealPips(PositionGetString(POSITION_SYMBOL), 75));
   }
